...
local function StartSpawning(inst)
	if inst.components.childspawner 
            and (GetSeasonManager() and not GetSeasonManager():IsWinter()) 
...
local function fn(Sim)
    ...
    inst:ListenForEvent("seasonChange", function(it, data)
        if data.season == SEASONS.SPRING then
            inst.components.childspawner:SetRegenPeriod(math.ceil(TUNING.BEEHIVE_REGEN_TIME / TUNING.SPRING_COMBAT_MOD))
            inst.components.childspawner:SetSpawnPeriod(math.ceil(TUNING.BEEHIVE_RELEASE_TIME / TUNING.SPRING_COMBAT_MOD))
            inst.components.childspawner:SetMaxChildren(math.ceil(TUNING.BEEHIVE_BEES * TUNING.SPRING_COMBAT_MOD))
        else
