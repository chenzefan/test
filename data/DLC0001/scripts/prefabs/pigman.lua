...
local function GuardRetargetFn(inst)
    ...
    if not defenseTarget and home and inst:GetDistanceSqToInst(home) < SpringMod(TUNING.PIG_GUARD_DEFEND_DIST*TUNING.PIG_GUARD_DEFEND_DIST) then
        defenseTarget = home
    end
    if not defenseTarget then
        defenseTarget = inst
    end
    local invader = FindEntity(defenseTarget or inst, SpringMod(TUNING.PIG_GUARD_TARGET_DIST), function(guy)
    ...
    return FindEntity(defenseTarget, SpringMod(TUNING.PIG_GUARD_DEFEND_DIST), function(guy)
...
local function GuardKeepTargetFn(inst, target)
    local home = inst.components.homeseeker and inst.components.homeseeker.home
    if home then
        local defendDist = SpringMod(TUNING.PIG_GUARD_DEFEND_DIST)
...
local function WerepigRetargetFn(inst)
    return FindEntity(inst, SpringMod(TUNING.PIG_TARGET_DIST), function(guy)
