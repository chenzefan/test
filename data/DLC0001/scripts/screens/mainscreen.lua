...
function MainScreen:DoInit( )
	...
	--center stuff
    self.shield = self.fixed_root:AddChild(Image("images/fepanels_DLC.xml", "panel_shield_DLC.tex"))
    ...
    self.banner:SetPosition(5, -210, 0)
	...
	self.updatename:SetString(STRINGS.UI.MAINSCREEN.DLC_UPDATENAME .. suffix)
	...
	if status and motd then
		...
		if PLATFORM == "WIN32_STEAM" or PLATFORM == "LINUX_STEAM" or PLATFORM == "OSX_STEAM" then
			if IsDLCInstalled(REIGN_OF_GIANTS) then
				platform_motd = motd.rogsteam
			else
				platform_motd = motd.steam
			end
		else
			if IsDLCInstalled(REIGN_OF_GIANTS) then
				platform_motd = motd.rogstandalone
			else
