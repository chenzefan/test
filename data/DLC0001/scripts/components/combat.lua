...
local Combat = Class(function(self, inst)
    ...
    self.onhitfn = nil
    self.onhitotherfnonhitotherfn = nil
    ...
    self.inst:AddTag("hascombatcomponent")    self.inst:AddTag("combat")
...
function Combat:ShareTarget(target, range, fn, maxnum)
    ...
    if GetSeasonManager() and GetSeasonManager():IsSpring() then
        ents = TheSim:FindEntities(x,y,z, range * TUNING.SPRING_COMBAT_MOD)
    else
        ents = TheSim:FindEntities(x,y,z, range)
    end
