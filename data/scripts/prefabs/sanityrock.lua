...
local function rockstartup(inst, player)
    if player and player.components.sanity and not player.components.sanity:IsSane() then
        ...
        inst.Physics:CollidesWith(COLLISION.ITEMS)
        turnoffpathfinding(inst)
    elseif player and player.components.sanity and player.components.sanity:IsSane() then
...
local function fn (Sim)
    ...
    anim:PlayAnimation("idle_active")
    -------------------------
    local player = GetPlayer()    
