...
local function SpawnScion(inst)
    ...
    if inst.style == 1 then
        spawn = (math.random()<.5 and "bishop_nightmare") or "rook_nightmare"
    elseif inst.style == 2 then
        spawn = (math.random()<.5 and "rook_nightmare") or "bishop_nightmare"
    else
        spawn = (math.random()<.5 and "rook_nightmare") or "bishop_nightmare"
    end
    ...
    if it and it.components.combat then
...
local function OnRepaired(inst)
	if inst.components.workable.workleft < MAXHITS then
	    ...
	    GetPlayer():DoTaskInTime(0.7, function() 
                                        inst.components.lootdropper:AddChanceLoot("gears",     0.5) 
                                        inst.components.lootdropper:AddChanceLoot("gears",     0.2) 
                                        if GetWorld() and GetWorld():IsCave() and GetWorld().topology.level_number == 2 then  -- ruins
                                            inst.components.lootdropper:AddChanceLoot("thulecite", 0.2)
                                        end
                                        inst.components.lootdropper:DropLoot()
                                        SpawnScion(inst)
                                        inst:Remove()
